...
export function courseCardHTML(course, { ctaLabel = 'View Course', ctaHref = null } = {}) {
  ...
  const subjectLevel = course.subjects?.level ?? '';
  const subjectLabel = subjectLevel ? `${subjectLevel} • ${subjectName}` : subjectName;
  return `
    <article class="card card-hover course-card">
      <a href="${href}" class="course-card__thumb" aria-hidden="true">
        ${course.thumbnail_url
          ...
          : `<span>${escapeHTML(subjectLabel || 'Course')}</span>`}
      </a>
      <div class="course-card__body">
        ${subjectLabel ? `<span class="badge badge-neutral">${escapeHTML(subjectLabel)}</span>` : ''}
